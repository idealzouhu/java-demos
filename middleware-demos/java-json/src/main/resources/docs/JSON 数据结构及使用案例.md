...
在这个例子中，我们创建了一个`User`对象，然后使用`ObjectMapper`的`writeValueAsString`方法将其转换为JSON字符串。`writeValueAsString`方法将  Java 对象转换为 JSON 字符串，这样我们就可以在需要时将其传递给其他系统、写入文件或发送到网络。
